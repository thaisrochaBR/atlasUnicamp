...
def get_s3_keys(bucket, prefix = ''):
    ...
    """
	#create account in AWS and generate your aws_access_key_id and secret_access_key
	#https://aws.amazon.com/pt/premiumsupport/knowledge-center/create-access-key/
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='This is a Unicamp NOAA-GOES script by Alison Vilela \ Thais Rocha. (Errors are not yet logged.)',
        prog='unicamp-noaa-goes',
    ...
    parser.add_argument('-p','--period',
        type=int,
        default=[10, 12, 14, 16],#default=[8, 10, 12, 14, 16],
...
def getimage(year, day, hour, outputpath, channel, crop):
    try:
        ...
        keys = get_s3_keys(bucket_name, prefix = product_name+'/'+ 
                            ...
                            product_name + '-M3C' + str(channel).zfill(2))
        ...
        with open(outputpath + '/pure/' + file_name + '.nc', 'rb') as in_file:
            ...
            plt.axis('off')
            plt.savefig(outputpath + '/' + file_name + '.png', dpi=300, facecolor='w', edgecolor='w', bbox_inches='tight')
            plt.close(fig)
