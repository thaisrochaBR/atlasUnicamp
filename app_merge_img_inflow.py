...
def gera_lista_arquivos_goes(ano1,dia1ini,dia1fim):
    dia=dia1ini
    dia_fim=dia1fim
    ano = ano1
    lstfiles=[]
...
def main():
    ...
    parser.add_argument('-e', "--endday",
        help="The end day of year format(365)",
        type=int)
    ...
    parser.add_argument('-a','--acronym',
        type=str,        
        help='name folder, acronym station (GAP_10_12h)')

    parser.add_argument('-g','--group',                
        type=str,
        help='media, 12h')

    parser.add_argument('-n','--ntl',
        type=int,        
        default=[0,1],
        help='number days of time lang [0,1]')
    ...
    if not args.startday:
        parser.error("argument -s/--startday: must have a date set on START")

    if not args.endday:
        parser.error("argument -e/--endday: must have a date set on END")

    if not args.csv:
        parser.error("argument -c/--csv: must have a date set on CSV")

    if not args.acronym:
        parser.error("argument -a/--acronym: must have a date set on Acronym")

    if not args.group:
        parser.error("argument -g/--group: must have a date set on Group")

    if not args.ntl:
        parser.error("argument -n/--ntl: must have a date set on NTL")

    data_dae = format_dados_vazao_DAE(args.csv)
    data_images = gera_lista_arquivos_goes(args.year,args.startday,args.endday)
    data_train,nome_csv = gera_csv_lista_dados_imagens_treinamento_rna(data_dae, data_images,args.acronym,args.group,args.ntl)
